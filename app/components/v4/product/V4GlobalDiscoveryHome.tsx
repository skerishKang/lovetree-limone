...
function mediaStyle(memory: MemoryRecord | undefined): CSSProperties | undefined {
  ...
  return source
    ? { backgroundImage: `linear-gradient(180deg,rgba(48,35,38,.02),rgba(48,35,38,.22)),url(${source})` }
    : undefined;
}
  ...
  return (
    ...
    >
      ...
      <span className="track01-card-info">
        <span className="track01-card-title-row">
        ...
        <span className="track01-card-tags">
          {(tree.emotions.length ? tree.emotions : ["기록"]).slice(0, 2).map((emotion) => <em key={emotion}>{emotion}</em>)}
        </span>
      </span>
    </button>
  );
...
export default function V4GlobalDiscoveryHome() {
  ...
  const visibleTrees = useMemo(() => {
    ...
    const rows = trees.filter((tree) => {
      ...
      return [tree.title, tree.memo, tree.artist, tree.groupName, ...tree.emotions]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(query);
    });
    if (sort === "moments") return [...rows].sort((a, b) => b.publicMoments.length - a.publicMoments.length);
    return rows;
  }, [filter, search, sort, trees]);

  useEffect(() => {
    if (!visibleTrees.length) return;
    if (!selectedTreeId || !visibleTrees.some((tree) => tree.id === selectedTreeId)) {
      setSelectedTreeId(visibleTrees[0].id);
      setSelectedMomentIndex(0);
    }
  }, [selectedTreeId, visibleTrees]);

  const selectedTree = trees.find((tree) => tree.id === selectedTreeId) || visibleTrees[0] || trees[0] || null;
    ...
    <main className="v4-global-discovery" data-track01-native="community">
      <div className="track01-page">
        <header className="track01-top">
          <Link className="track01-brand" href="/v4">
            <span className="track01-mark" aria-hidden="true"><i /><b /><em /></span>
            <span><strong>러브트리</strong><small>LOVETREE</small></span>
          </Link>
          <nav aria-label="LoveTree 주요 메뉴">
            <Link href="/v4">첫 화면</Link>
            <Link href="/v4#about">LoveTree 소개 보기</Link>
            <Link className="is-active" href="/v4/community" aria-current="page">둘러보기</Link>
            <Link href="/v4?start=1">내 러브트리 시작하기</Link>
          </nav>
        ...
        <div className="track01-main">
          <section className="track01-discovery" aria-labelledby="track01-title">
            <div className="track01-intro">
              <p className="track01-kicker">공개 러브트리 둘러보기</p>
              <h1 id="track01-title">마음이 머무는 <em>순간들을</em><br />천천히 감상해보세요</h1>
              <p>다른 사람들이 이어간 첫 순간과 감정의 흐름을 둘러보고, 마음이 닿는 러브트리를 천천히 만나보세요.</p>
            </div>

            <div className="track01-controls">
              <label className="track01-search">
                <span aria-hidden="true">⌕</span>
                <input value={search} onChange={(event) => setSearch(event.currentTarget.value)} type="search" placeholder="인물, 트리 제목, 감정으로 찾아보세요" aria-label="공개 러브트리 검색" />
              </label>
              <div className="track01-filters" aria-label="감정 필터">
                {availableFilters.map((item) => (
                  <button type="button" key={item} className={filter === item ? "is-active" : ""} onClick={() => setFilter(item)}>{item}</button>
                ))}
              </div>
              <select value={sort} onChange={(event) => setSort(event.currentTarget.value as SortMode)} aria-label="공개 러브트리 정렬">
                <option value="popular">인기순</option>
                <option value="latest">최근 공개순</option>
            ...
            {loading ? <div className="track01-empty">공개 러브트리를 불러오는 중입니다.</div> : error ? <div className="track01-empty" role="alert">{error}</div> : visibleTrees.length ? (
              <div className="track01-grid">
                {visibleTrees.map((tree, index) => (
                  <Track01TreeCard
                    key={tree.id}
                    tree={tree}
                    selected={selectedTree?.id === tree.id}
                    palette={paletteFor(tree, trees.findIndex((item) => item.id === tree.id) >= 0 ? trees.findIndex((item) => item.id === tree.id) : index)}
                    onSelect={() => selectTree(tree)}
                  />
                ))}
              </div>
            ) : <div className="track01-empty">찾는 마음의 트리가 아직 없어요.<br />다른 검색어나 감정을 골라보세요.</div>}
            <div className="track01-more"><button type="button" onClick={() => router.push("/v4/community")}>더 많은 공개 러브트리 보기⌄</button></div>
          </section>

          <aside className="track01-preview-wrap" ref={previewRef}>
            {selectedTree ? (
              <article
                className="track01-book"
                data-track01-preview
                style={{ "--track-accent": selectedPalette.accent, "--track-pale": selectedPalette.pale } as CSSProperties}
              >
                <div className="track01-detail">
                  <p>❧ 이 러브트리에 담긴 마음</p>
                  <h2>{selectedTree.title}</h2>
                  <span>{selectedTree.memo || "공개된 순간들이 하나의 흐름으로 이어진 러브트리입니다."}</span>
                </div>

                <div className="track01-collage" aria-label="선택한 러브트리 대표 순간 미리보기">
                  <span className="track01-branch" aria-hidden="true" />
                  <i className="track01-leaf leaf-a" aria-hidden="true" /><i className="track01-leaf leaf-b" aria-hidden="true" /><i className="track01-leaf leaf-c" aria-hidden="true" />
                  ...
                  <button type="button" className="track01-paper track01-hero-paper" onClick={() => firstMoment && router.push(`/trees/${encodeURIComponent(selectedTree.id)}?moment=${encodeURIComponent(firstMoment.id)}`)}>
                    <small>01</small><span className="track01-paper-media" style={mediaStyle(firstMoment)}><i aria-hidden="true">▶</i></span><strong>{safeMomentTitle(firstMoment, "첫 번째 공개 순간")}</strong><em aria-hidden="true">L</em>
                  </button>
                  <button type="button" className="track01-paper track01-small-paper" onClick={() => secondMoment && router.push(`/trees/${encodeURIComponent(selectedTree.id)}?moment=${encodeURIComponent(secondMoment.id)}`)}>
                    <small>02</small><span className="track01-paper-media" style={mediaStyle(secondMoment)}><i aria-hidden="true">▶</i></span><strong>{safeMomentTitle(secondMoment, "이어진 공개 순간")}</strong>
                  </button>
      ...
      <dialog
        ref={dialogRef}
        className="track01-dialog"
        data-track01-full-tree
        onCancel={(event) => { event.preventDefault(); closeFullTree(); }}
        onClose={() => setFullTreeOpen(false)}
      >
        {selectedTree ? (
          <section className="track01-tree-shell" style={{ "--track-accent": selectedPalette.accent, "--track-pale": selectedPalette.pale } as CSSProperties}>
            <header className="track01-toolbar">
              ...
              <div className="track01-zoom" aria-label="트리 확대/축소">
                <button type="button" onClick={() => setZoom((value) => Math.max(.65, Number((value - .1).toFixed(2))))} aria-label="축소">−</button>
                <button type="button" onClick={() => setZoom(1)}>맞춤</button>
                <button type="button" onClick={() => setZoom((value) => Math.min(1.25, Number((value + .1).toFixed(2))))} aria-label="확대">＋</button>
              </div>
            ...
            <div className="track01-tree-body">
              <div className="track01-canvas-wrap">
                <div className="track01-canvas" style={{ transform: `scale(${zoom})` }}>
                  <svg viewBox="0 0 1000 680" aria-hidden="true">
                    {selectedTree.publicMoments.slice(0, 8).map((moment, index) => {
                      const position = TREE_POSITIONS[index] || TREE_POSITIONS[TREE_POSITIONS.length - 1];
                      return <path key={moment.id} d={`M 500 340 C ${500 + (position.x - 500) * .55} 340, ${500 + (position.x - 500) * .55} ${position.y + 70}, ${position.x + 90} ${position.y + 70}`} />;
                    })}
                  </svg>
                  ...
                  {selectedTree.publicMoments.slice(0, 8).map((moment, index) => {
                    ...
                    return (
                      <button
                        type="button"
                        key={moment.id}
                        className={`track01-moment-node${selectedMoment?.id === moment.id ? " is-selected" : ""}`}
                        style={{ left: `${position.x / 10}%`, top: `${position.y / 6.8}%` }}
                        data-track01-moment-node
                        onClick={() => setSelectedMomentIndex(index)}
                        aria-pressed={selectedMoment?.id === moment.id}
                      >
                        <span style={mediaStyle(moment)}><i>{String(index + 1).padStart(2, "0")}</i></span>
                        <strong>{moment.title || `공개 순간 ${index + 1}`}</strong>
                        <small>{sourceTypeLabel(moment.sourceType)}</small>
                      </button>
                    );
                  })}
                </div>
              ...
              <aside className="track01-panel">
                <p>SELECTED MOMENT</p>
                <h3>{selectedMoment?.title || "공개 순간"}</h3>
                <small>{selectedMoment ? `${selectedMomentIndex + 1}번째 순간 · ${sourceTypeLabel(selectedMoment.sourceType)}` : ""}</small>
