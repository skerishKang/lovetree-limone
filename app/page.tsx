...
export default function Home() {
  ...
  async function startTree(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await apiFetch("/api/trees", {
        method: "POST",
        body: JSON.stringify({ title: treeName }),
      });
      if (res.ok) {
  ...
  async function plantMoment(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      await apiFetch(`/api/trees/${currentTreeId}/memories`, {
        method: "POST",
        body: JSON.stringify({ kind: selectedKind, note }),
      });
    } finally {
      setSaving(false);
    }
