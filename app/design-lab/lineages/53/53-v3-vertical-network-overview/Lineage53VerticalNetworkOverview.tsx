...
function pointOnPrimary(
  ...
  const a = angle + bend;
  return {
    x: snap(center.x + Math.cos(a) * radius + Math.sin(step * 1.7 + pathIndex) * 12),
    y: snap(center.y + Math.sin(a) * radius * 0.8 + Math.cos(step * 1.3 + pathIndex) * 10),
  };
}

/**
 * Coordinates are rounded to a stable precision so server-rendered and
 * client-rendered SVG attributes are identical (hydration safety).
 */
      ...
      path.secondaryBranches.forEach((branch, branchIndex) => {
        ...
        branch.momentIds.forEach((id, step) => {
          ...
          nodes.set(id, {
            ...
            pathIndex,
            hierarchy: "secondary",
            x: snap(parent.x + Math.cos(branchAngle) * distance),
            y: snap(parent.y + Math.sin(branchAngle) * distance * 0.78),
          });
        });
      });
    });
...
function edgePath(edge: LayoutEdge) {
  ...
  const side = dx >= 0 ? 1 : -1;
    const p = (v: number) => String(snap(v));
  return `M ${p(edge.from.x)} ${p(edge.from.y)} C ${p(edge.from.x + bend * side)} ${p(edge.from.y + dy * 0.22)}, ${p(edge.to.x - bend * side)} ${p(edge.to.y - dy * 0.22)}, ${p(edge.to.x)} ${p(edge.to.y)}`;
}
  ...
  return (
    <section className="s56" data-reduced-motion={reducedMotion ? "true" : "false"} data-network-mode={mode}>
      ...
      <div className="s56-stage" aria-label="Source56 vertical Moment network overview">
        ...
        {selected ? (
          <aside className={`s56-inspector open${selected.first ? " compact-origin" : ""}`} aria-label="Selected Moment inspector">
