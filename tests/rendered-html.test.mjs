...
test("keeps finished-site assets and metadata wired", async () => {
  ...
  assert.match(page, /삭제하고 가지 정리하기/);
  assert.match(page, /첫 마음이/);
  assert.match(page, /한 그루가 됐어요/);
  assert.match(page, /꽃이 피기 시작했어요/);
  assert.match(page, /열매를 맺고 있어요/);
  assert.match(page, /무성하게 자라고 있어요/);
  assert.match(page, /tree-growth-flower/);
  assert.match(page, /tree-growth-fruit/);
  assert.match(page, /기억할 시각 5초 줄이기/);
  assert.match(page, /기억할 시각 5초 늘리기/);
  assert.match(page, /다른 마음/);
  assert.match(page, /공개할 때 이 메모/);
  assert.match(page, /두 장면 연결 미리보기/);
