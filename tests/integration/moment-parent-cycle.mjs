...
async function main() {
  ...
  const firstMemoryKey = `cycle-first-${Date.now()}`;
  const firstMemory = await createTreeWithFirstMemory({
    clientKey: firstMemoryKey,
    title: "First Tree",
    memory: { title: "First Moment" },
  });
  record("first-memory without parent succeeds", firstMemory.status, 201);
  const firstMemoryReplay = await createTreeWithFirstMemory({
    clientKey: firstMemoryKey,
    title: "Changed Replay Title",
    memory: { title: "Changed Replay Memory", parentId: "b" },
  });
  record("first-memory replay preserves original status", firstMemoryReplay.status, 200);
  record(
    "first-memory replay preserves original parent",
    firstMemoryReplay.body?.memory?.parentId,
    null
  );
  ...
  // Existing owner-scoped tree move semantics remain available; the helper
  // locks both source and target Trees while the guarded write runs.
  const move = await put("move", { treeId: "tree-b", parentId: "cross" });
  record("tree move with target parent status", move.status, 200);
  record("tree move persisted tree", move.body?.treeId, "tree-b");
  record("tree move persisted parent", move.body?.parentId, "cross");
  const moveBack = await put("move", { treeId: "tree-a", parentId: null });
  record("tree move detach status", moveBack.status, 200);
  record("tree move detach persisted tree", moveBack.body?.treeId, "tree-a");
  record("tree move detach persisted parent", moveBack.body?.parentId, null);
