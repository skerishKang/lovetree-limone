...
test("B. INSPECT_FREEZE — world state frozen during inspect dialog", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("B-inspect-freeze.json", EVIDENCE_DIR)),
...
test("C. INSPECT_DIALOG — inspect dialog semantics and visible state", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("C-inspect-dialog.json", EVIDENCE_DIR)),
...
test("D. INSPECT_FOCUS — focus moves to inspect dialog on open", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("D-inspect-focus.json", EVIDENCE_DIR)),
...
test("E. INSPECT_ESCAPE — Escape key closes inspect dialog", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("E-inspect-escape.json", EVIDENCE_DIR)),
...
test("F. INSPECT_RESTORE — canvas/runtime restores after inspect close", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("F-inspect-restore.json", EVIDENCE_DIR)),
...
test("G. INSPECT_HIGH_RES — inspect viewer shows high-resolution source asset", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("G-inspect-high-res.json", EVIDENCE_DIR)),
...
test("H. TOUCH_390x844 — genuine touchscreen tap at mobile viewport", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("H-touch-390x844.json", EVIDENCE_DIR)),
...
test("I. TOUCH_320x720 — genuine touchscreen tap at small mobile viewport", async () => {
  ...
  try {
    ...
    await writeFile(
      fileURLToPath(new URL("I-touch-320x720.json", EVIDENCE_DIR)),
