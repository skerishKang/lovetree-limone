...
test("Source56 is a bounded Lineage53 extension and never Lineage56", async () => {
...
test("canonical proof data is Moment and Connection; PathFamily is derived presentation", () => {
  assert.equal(SOURCE56_MOMENTS.length, 31);
  assert.equal(SOURCE56_CONNECTIONS.length, 30);
  ...
  assert.equal(new Set(families.map((family) => family.seedMomentId)).size, 6);
  assert.ok(families.every((family) => family.primaryMomentIds.length === 4));
  assert.ok(families.every((family) => family.secondaryMomentIds.length === 1));
});

test("native component reuses P3/P4 and creates no source runner or persistence", async () => {
