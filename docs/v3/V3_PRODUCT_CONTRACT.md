...
- V2: `/v2/**` 의 기능 업그레이드 제품 (별도 후보로 유지, `main`에 통합됨)
- V3: `/v3/**` 의 "사랑 연혁" 제품 후보 (이 브랜치에서 진행)

V1·V2·V3는 각각 별도 후보로 유지되며, canonical 제품 선정은 별도 승인 결정에 따릅니다.
`docs/architecture/V3_DOMAIN_MODEL_DECISION.md`는 관련 결정 초안이지만, 해당 문서는
미병합 PR #25의 변경 범위에 있으므로 이 계약에서는 **pending dependency**로만 취급하며
canonical authority로 간주하지 않습니다.
