...
async function captureStatePair(browser, viewport, state) {
  const source = await openSource(browser, viewport);
  const native = await openNative(browser, viewport);
  await assertNoOverflow(source.page, `source-${viewport.name}-${state}`);
  await assertNoOverflow(native.page, `native-${viewport.name}-${state}`);
  await assertNoHeaderOverlap(native.page, `${viewport.name}-${state}`);
  ...
  const sourceBuffer = await capture(source.page, `source-${viewport.name}-${state}.png`);
  const nativeBuffer = await capture(native.page, `native-${viewport.name}-${state}.png`);
  await compare(browser, sourceBuffer, nativeBuffer, viewport, `compare-${viewport.name}-${state}.png`);
  await validateErrors(source, `source-${viewport.name}-${state}`);
  await validateErrors(native, `native-${viewport.name}-${state}`);
...
try {
  for (const viewport of viewports) {
    await captureStatePair(browser, viewport, "initial");
    await captureStatePair(browser, viewport, "next");
  }
  await captureReduced(browser);
  console.log("TRACK37_SOURCE_NATIVE_VISUAL_FIDELITY_V2=PASS");
} finally {
  await browser.close();
}
