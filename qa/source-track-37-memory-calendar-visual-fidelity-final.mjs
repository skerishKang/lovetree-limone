...
async function openSource(browser, viewport, reducedMotion = "no-preference") {
  const context = await browser.newContext({ viewport: { width: viewport.width, height: viewport.height }, hasTouch: viewport.mobile, isMobile: viewport.mobile, reducedMotion });
...
async function openNative(browser, viewport, reducedMotion = "no-preference") {
  ...
  await page.getByRole("button", { name: /^01\s*2$/ }).click();
  await page.getByText("2026.08.01", { exact: true }).waitFor();
  return { context, page, ...errors };
}

async function assertNoOverflow(page, label) {
  const dims = await page.evaluate(() => ({ inner: window.innerWidth, scroll: document.documentElement.scrollWidth }));
  assert.ok(dims.scroll <= dims.inner, `${label}: horizontal overflow ${dims.scroll} > ${dims.inner}`);
}
...
async function compare(browser, leftBuffer, rightBuffer, viewport, filename) {
...
async function validateErrors(session, label) {
  assert.deepEqual(session.pageErrors, [], `${label}: page errors ${session.pageErrors.join(" | ")}`);
  assert.deepEqual(session.consoleErrors, [], `${label}: console errors ${session.consoleErrors.join(" | ")}`);
}

async function captureInitialPair(browser, viewport) {
  const source = await openSource(browser, viewport);
  const native = await openNative(browser, viewport);
  await assertNoOverflow(source.page, `source-${viewport.name}`);
  await assertNoOverflow(native.page, `native-${viewport.name}`);
  await assertNoHeaderOverlap(native.page, viewport.name);
  const sourceBuffer = await capture(source.page, `source-${viewport.name}-initial.png`);
  const nativeBuffer = await capture(native.page, `native-${viewport.name}-initial.png`);
  await compare(browser, sourceBuffer, nativeBuffer, viewport, `compare-${viewport.name}-initial.png`);
  await validateErrors(source, `source-${viewport.name}`);
  await validateErrors(native, `native-${viewport.name}`);
  await source.context.close();
  await native.context.close();
}

async function captureDesktopNextPair(browser) {
  const viewport = viewports[0];
  const source = await openSource(browser, viewport);
  const native = await openNative(browser, viewport);
  const demo = source.page.locator("#demoButton");
  await assert.rejects(async () => { if (!(await demo.isVisible())) throw new Error("not-visible"); }, () => false).catch(() => {});
  assert.equal(await demo.isVisible(), true, "desktop source tear demo control must be visibly available");
  await demo.click();
  await source.page.waitForTimeout(1500);
  const day10 = native.page.getByRole("button", { name: /^10\s*1$/ });
  await day10.click();
  await native.page.getByText("2026.08.10", { exact: true }).waitFor();
  const sourceBuffer = await capture(source.page, `source-${viewport.name}-next.png`);
  const nativeBuffer = await capture(native.page, `native-${viewport.name}-next.png`);
  await compare(browser, sourceBuffer, nativeBuffer, viewport, `compare-${viewport.name}-next.png`);
  await validateErrors(source, "source-desktop-next");
  await validateErrors(native, "native-desktop-next");
  await source.context.close();
  await native.context.close();
}
...
async function captureReduced(browser) {
  ...
  await assertNoOverflow(source.page, "source-reduced-motion");
  await assertNoOverflow(native.page, "native-reduced-motion");
  await assertNoHeaderOverlap(native.page, "reduced-motion");
  const animationName = await native.page.getByTestId("track37-active-pad").evaluate((node) => getComputedStyle(node).animationName);
  assert.ok(animationName === "none" || animationName === "", `reduced-motion native tear animation must be disabled, got ${animationName}`);
  const sourceBuffer = await capture(source.page, `source-${viewport.name}.png`);
  const nativeBuffer = await capture(native.page, `native-${viewport.name}.png`);
  await compare(browser, sourceBuffer, nativeBuffer, viewport, `compare-${viewport.name}.png`);
  await validateErrors(source, "source-reduced-motion");
  await validateErrors(native, "native-reduced-motion");
  await source.context.close();
  await native.context.close();
}
...
try {
  for (const viewport of viewports) await captureInitialPair(browser, viewport);
  await captureDesktopNextPair(browser);
  await captureReduced(browser);
  console.log("TRACK37_SOURCE_NATIVE_VISUAL_FIDELITY_FINAL=PASS");
} finally {
  await browser.close();
}
